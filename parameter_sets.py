...
# ===================================================================================
# a set of parameters for network training
# ===================================================================================
...
def parameter_set1():
    ...
    path_mask = path_curr_dir + '/Mat_Files/maskRec_di.mat'
    ...
    lamda = 0.668  # mean emission wavelength # in [um] (1e-6*meter)
    NA = 1.35  # numerical aperture of the objective lens
    noil = 1.406  # immersion medium refractive index
    nwater = 1.33  # imaging medium refractive index
    pixel_size_CCD = 13  # sensor pixel size in [um]
    pixel_size_SLM = 20  # SLM pixel size in [um] (after binning of 3 to reduce computational complexity)
    M = 100  # optical magnification
    f_4f = 20e4  # 4f lenses focal length in [um]
    ...
    Hmask, Wmask = mask_init.shape[0], mask_init.shape[1]  # in SLM [pixels] match with
    ...
    zmax = 6.5  # maximal z in [um] (including the axial shift)
    ...
    blur_std_range = [0.49, 0.61]  # in sensor [pixels]
    ...
    read_noise_baseline_range = [10.0, 20.0]  # in [counts]

    # read noise standard deviation upper and lower range
    read_noise_std_range = [6.0, 16.0]  # in [counts]
    ...
    training_data_path = path_curr_dir + "/Training_Data/TrainingImages1/"
    ...
    results_path = path_curr_dir + "/Training_Results/Results1/"
...
def parameter_set2():
    ...
    # initial mask for learning an optimized mask or final mask for training a localization model
    # if learn_mask=True the initial mask is initialized by default to be zero-modulation
    path_mask = path_curr_dir + '/Mat_Files/maskRec.mat'
    mask_dict = sio.loadmat(path_mask)
    mask_name = list(mask_dict.keys())[3]
    mask_init = mask_dict[mask_name]
    ...
    lamda = 0.680  # mean emission wavelength # in [um] (1e-6*meter)
    NA = 1.35  # numerical aperture of the objective lens
    noil = 1.406  # immersion medium refractive index
    nwater = 1.33  # imaging medium refractive index
    pixel_size_CCD = 13  # sensor pixel size in [um]
    ...
    # discretization in z
    D = 91  # in [voxels] spanning the axial range (zmax - zmin)
    ...
    # range of baseline of the min-subtracted data in STORM
    read_noise_baseline_range = [46.0, 70.0]  # in [counts]

    # read noise standard deviation upper and lower range
    read_noise_std_range = [26.0, 40.0]  # in [counts]
    ...
    # path for saving training examples: images + locations for localization net or locations + photons for PSF learning
    training_data_path = path_curr_dir + "/Training_Data/TrainingImages3/"
    ...
    # results folder to save the trained model
    results_path = path_curr_dir + "/Training_Results/Results3/"
