...
from Summplement.sfs import image_overlay_rendering


def crop_stitch(H, W, clear_dist, HH, WW):
    """
    better to not pad zeros
    give the valid H and W for each sub area
    the center is the image center, going right is the positive x, going down is the positive y

    details for padding, cropping, stitching
    H-height of the cropped image/network input
    W-width of the cropped image/network input
    clear_dist- margin of the cropped image/network input
    HH-hight of the experimental image
    WW-width of the experimental image
    return: pcs_dict
    """

    H_valid, W_valid = H - 2 * clear_dist, W - 2 * clear_dist  # valid height and width of each patch
    num_patches_H = int((HH - H) // H_valid)+1  # how many patches in each column
    if ((HH - H) % H_valid) > clear_dist:  # if the mode is larger than clear_dist, one more
        num_patches_H = num_patches_H + 1
    num_patches_W = int((WW - W) // W_valid)+1  # how many patches in each row
    if ((WW - W) % W_valid) > clear_dist:
        num_patches_W = num_patches_W + 1

    cs_dict = dict()  # dictionary for cropping and stitching
    # cs_dict['H_valid'], cs_dict['W_valid'] = H_valid, W_valid
    cs_dict['H'], cs_dict['W'] = H, W
    cs_dict['HH'], cs_dict['WW'] = HH, WW
    # pcs_dict['HH_pad'], pcs_dict['WW_pad'] = HH_pad, WW_pad
    cs_dict['num_patches_H'], cs_dict['num_patches_W'] = num_patches_H, num_patches_W

    for i in range(num_patches_H):
        for j in range(num_patches_W):
            dict_key = 'r' + str(i + 1) + 'c' + str(j + 1)

            row_start, row_end = i * H_valid, i * H_valid + H
            y_center = (row_start + row_end-1)/2
            valid_y0, valid_y1 = -H/2+clear_dist, H/2-clear_dist  # initial valid y range (pixels) in local coordinates
            if row_end > HH:
                row_end = HH  # shift
                y_range = HH-clear_dist-(row_start+clear_dist)  # valid range to cover
                row_start = row_end-H  # update
                valid_y0 = valid_y1-y_range  # crucial
                y_center = (row_start + row_end-1)/2  # global
            column_start, column_end = j * W_valid, j * W_valid + W
            x_center = (column_start+column_end-1)/2
            valid_x0, valid_x1 = -W/2+clear_dist, W/2-clear_dist
            if column_end > WW:
                column_end = WW
                x_range = WW-clear_dist-(column_start+clear_dist)
                column_start = column_end-W
                valid_x0 = valid_x1-x_range
                x_center = (column_start+column_end-1)/2  # global

            # for each patch, record the cropping row start and end, cropping column start and end,
            # local valid x and y range, and the global center
            cs_dict[dict_key] = (row_start, row_end, column_start, column_end,  # cropping
                                 valid_x0, valid_x1, valid_y0, valid_y1, x_center, y_center,  # localization filtering
                                 x_center+valid_x0, x_center+valid_x1, y_center+valid_y0, y_center+valid_y1)  #stitching
    # check the crop and stitch
    plt.figure(1123)
    # draw the frame edges
    plt.plot([-WW/2, WW/2], [-HH/2, -HH/2], color='blue')
    plt.plot([WW / 2, WW / 2], [-HH/2, HH/2], color='blue')
    plt.plot([WW / 2, -WW / 2], [HH / 2, HH / 2], color='blue')
    plt.plot([-WW / 2, -WW / 2], [HH / 2, -HH / 2], color='blue')
    for i in range(num_patches_H):
        for j in range(num_patches_W):
            # draw valid areas
            dict_key = 'r' + str(i + 1) + 'c' + str(j + 1)
            patch_params = cs_dict[dict_key]
            valid_x0, valid_x1, valid_y0, valid_y1, x_center, y_center = patch_params[4:10]
            x_center, y_center = x_center-WW/2, y_center-HH/2  # center shift

            plt.plot([x_center+valid_x0, x_center+valid_x1], [y_center+valid_y0, y_center+valid_y0], color='red')
            plt.plot([x_center + valid_x1, x_center + valid_x1], [y_center + valid_y0, y_center + valid_y1],
                     color='red')
            plt.plot([x_center + valid_x1, x_center + valid_x0], [y_center + valid_y1, y_center + valid_y1],
                     color='red')
            plt.plot([x_center + valid_x0, x_center + valid_x0], [y_center + valid_y1, y_center + valid_y0],
                     color='red')
    plt.title('cropping demonstration')
    plt.draw()
    plt.pause(0.05)
    return cs_dict


def test_model(path_results, postprocess_params, device, exp_imgs_path=None, seed=68):
    ...
    # network
    path_learning_pickle = path_results + 'learning_results.pickle' # load learning results
    ...
    # inference
    if exp_imgs_path is None:  # if no experimental images are supplied then sample a random example
        ...
        if setup_params['nsig_unif'] is False:
            ...
            if np.not_equal(nemitters, 1):
                nphotons_gt = np.squeeze(nphotons_gt.cpu().numpy(), 0)
        ...
        jaccard_index, RMSE_xy, RMSE_z, _ = calc_jaccard_rmse(xyz_gt, xyz_rec, 0.15)
        ...
        if len(img_names) == 1:
            ...
            # turn recovered positions into phases
            xyz_rec_ = np.expand_dims(xyz_rec, 0)
            phases_np = EmittersToPhases(xyz_rec_, setup_params)
            phases_emitter_rec = complex_to_tensor(phases_np).to(device)

            # use a uniform number of photons for recovery visualization
            nphotons_rec = 8e3 * torch.ones((1, xyz_rec_.shape[1])).to(device)
            ...
            im_base = np.squeeze(exp_tensor.cpu().numpy())
            im_overpay = np.squeeze(exp_pred_im.cpu().numpy())
            image_overlay_rendering(im_base, im_overpay)
...
def test_model_stitching(path_results, postprocess_params, device, exp_imgs_path=None, seed=66):
    ...
    cnn.to(device)
    cnn.load_state_dict(torch.load(path_results + 'weights_best_loss.pkl', map_location=device))
    thresh, radius = postprocess_params['thresh'], postprocess_params['radius']    # post-processing module on CPU/GPU
    postprocessing_module = Postprocess(thresh, radius, setup_params)
    ...
    # determine details of padding, cropping, and stitching
    H, W, clear_dist = setup_params['H'], setup_params['W'], setup_params['clear_dist']
    exp_im = imread(img_names[0])
    HH, WW = exp_im.shape[0], exp_im.shape[1]
    cs_dict = crop_stitch(H, W, clear_dist, HH, WW)  # crop and stitch

    cnn.eval()
    if len(img_names) == 1:  # if given only 1 image then show xyz in 3D and recovered image
        # ==========================================================================================================
        # read experimental image and normalize it
        # ==========================================================================================================
        # read exp image in  uint16
        ...
        with torch.set_grad_enabled(False):
            ...
            conf_rec = None
            tpost_start = time.time()
            for row_idx in range(cs_dict['num_patches_H']):
                for col_idx in range(cs_dict['num_patches_W']):
                    dict_key = 'r' + str(row_idx + 1) + 'c' + str(col_idx + 1)
                    patch_params = cs_dict[dict_key]
                    patch = exp_img[patch_params[0]:patch_params[1], patch_params[2]:patch_params[3]]
                    valid_x0, valid_x1, valid_y0, valid_y1, x_center, y_center = patch_params[4:10]
                    valid_x0 = valid_x0 * setup_params['pixel_size_FOV']
                    valid_x1 = valid_x1 * setup_params['pixel_size_FOV']
                    valid_y0 = valid_y0 * setup_params['pixel_size_FOV']
                    valid_y1 = valid_y1 * setup_params['pixel_size_FOV']
                    x_center = (x_center-WW/2) * setup_params['pixel_size_FOV']
                    y_center = (y_center-HH/2) * setup_params['pixel_size_FOV']

                    # normalize image according to the training setting
                    if setup_params['project_01'] is True:
                        patch = normalize_01(patch)
                    else:
                        patch = (patch - setup_params['global_factors'][0]) / setup_params['global_factors'][1]

                    # alter the mean and std to match the training set
                    if setup_params['project_01'] is True:
                        patch = (patch - patch.mean()) / patch.std()
                        patch = patch * setup_params['train_stats'][1] + setup_params['train_stats'][0]
                    ...
                    pred_volume = cnn(patch_tensor)

                    # post-process predicted volume

                    sub_xyz_rec, sub_conf_rec = postprocessing_module(pred_volume)

                    # filter and concatenate, according to valid_xy
                    if sub_xyz_rec is not None:
                        xx, yy = sub_xyz_rec[:, 0], sub_xyz_rec[:, 1]
                        tfx = (xx >= valid_x0) & (xx < valid_x1)
                        tfy = (yy >= valid_y0) & (yy < valid_y1)
                        valid_indices = np.where(tfx & tfy)[0]
                        if valid_indices.shape[0] != 0:  # valid localization exists
                            sub_xyz_rec = sub_xyz_rec[valid_indices, :]
                            sub_conf_rec = sub_conf_rec[valid_indices]
                            sub_xyz_rec[:, 0] = sub_xyz_rec[:, 0] + x_center
                            sub_xyz_rec[:, 1] = sub_xyz_rec[:, 1] + y_center
        ...
        tpost_elapsed = time.time() - tpost_start
        print('Post-processing of one frame complete in {:.6f}s'.format(tpost_elapsed))
        ...
        plt.figure(1532)
        plt.imshow(exp_im)
        plt.plot(xyz_rec[:, 0] / setup_params['pixel_size_FOV'] + WW / 2,
                 xyz_rec[:, 1] / setup_params['pixel_size_FOV'] + HH / 2, 'r*')
        plt.draw()
        plt.pause(0.05)
        ...
        H, W = exp_im.shape
        setup_params['H'], setup_params['W'] = H, W
        # The following might be a problem when it comes to a large FOV, because the simulation size is limited
        # by the phase mask whose size shouldn't be too large for the sake of simulation time efficiency
        psf_module_vis = PhysicalLayerVisualization(setup_params, 0, 0, 1)
        ...
        im_base = exp_im
        im_overpay = np.squeeze(exp_pred_im.data.cpu().numpy())
        image_overlay_rendering(im_base, im_overpay)
        ...
        results = np.array(['frame', 'x [nm]', 'y [nm]', 'z [nm]', 'intensity [au]'])
        with torch.set_grad_enabled(False):
            for im_ind, exp_im_path in enumerate(img_names):
                exp_im = imread(exp_im_path)  # read frame by frame
                exp_img = exp_im.astype("float32")
